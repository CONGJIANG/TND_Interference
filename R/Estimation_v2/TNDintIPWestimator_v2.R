...
# ?? why in r having 1/alpha or 1/(1-alpha)?
# ??include_alpha=T 
# ?? same thing in the denomenator.
CalcNumerator<-function(Ai_j, Xi_j, gamma_numer, alpha, re_alpha,
                        include_alpha = TRUE) {
  
  # ***
  gamma_numer[1]<-re_alpha
  # ***
...
CalcNumerator_old1 <- function(Ai_j, Xi_j, gamma_numer, alpha, re_alpha,
                          include_alpha = TRUE) {
...
GroupIPW_old1<-function(dta, cov_cols, phi_hat, gamma_numer = NULL, alpha,
                   neigh_ind = NULL, trt_col = NULL, out_col = NULL, 
                   alpha_re_bound = 10, integral_bound = 10,
                   keep_re_alpha = FALSE, estimand = c('GLMM', 'TypeB'),
                   verbose = TRUE) {
...
IEvar_old1 <- function(ygroupV, a1, a2, scores){
